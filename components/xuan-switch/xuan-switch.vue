<template>
	<view class="switch-container" :style="[{ background: bj_color}]">
		<view class="switch_view">
			...
			<view class="switch-item" :class="{'checked_switch':!isSwitch}"
				:style="!isSwitch?`color:${checked_color}`:''" @click.prevent.stop="changeSwitch(false)"
...
<script>
	export default {
		...
		watch: {
			defaultSwitch: { //深度监听，可监听到对象、数组的变化
				handler(val) {
		...
		methods: {
			...
			callParentEvent() {
				this.$emit("change", this.isSwitch, this.id, (isSuccess = true) => {
